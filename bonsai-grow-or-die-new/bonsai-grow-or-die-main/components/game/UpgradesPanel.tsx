...
function getEffectDescription(tier: UpgradeTierDef, trackKey: string, year: number) {
  ...
  if (tier.prerollPricePerLb) effects.push({ text: `Trim sold at $${tier.prerollPricePerLb}/lb on harvest`, detail: `33% of harvest yield → pre-roll revenue (instant payout)` });
  ...
  return (
    <div>
      ...
      {Object.entries(UPGRADE_TRACKS).map(([key, track]) => {
        ...
        const eligibleRooms = key === "preroll" ? unlockedRooms.filter(r => r.type === "flower") : unlockedRooms;
        const roomTiers = eligibleRooms.map(r => getRoomUpgradeTier(upgrades, key, r.index));
        const minTier = roomTiers.length > 0 ? Math.min(...roomTiers) : 0;
        const maxTier = roomTiers.length > 0 ? Math.max(...roomTiers) : 0;
        const roomsByMaxTier = eligibleRooms.filter(r => getRoomUpgradeTier(upgrades, key, r.index) >= totalTiers).length;
        const allMaxed = eligibleRooms.length > 0 && roomsByMaxTier === eligibleRooms.length;
          ...
          <div key={key} style={{
            ...
            {!allMaxed && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 6, marginTop: 4 }}>
                {unlockedRooms
                  .filter(room => key !== "preroll" || room.type === "flower")
                  .map(room => {
